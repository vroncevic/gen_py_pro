...
declare -A GEN_PY_PRO_USAGE=(
	...
	[USAGE_ARG1]="[PNAME] Python App project name"
...
function __gen_py_pro() {
	...
	if [ -n "${PNAME}" ]; then
		...
		MSG="Generate project root directory [${PNAME}/]!"
		__info_debug_message "$MSG" "$FUNC" "$GEN_PY_PRO_TOOL"
		mkdir "${PNAME}/"
		local SRC="${PNAME}/src"
		MSG="Generate source directory [${SRC}/]"
		__info_debug_message "$MSG" "$FUNC" "$GEN_PY_PRO_TOOL"
		mkdir "${SRC}/"
		local PROJECT_SET=${config_gen_py_pro_util[PROJECT_SET]}
